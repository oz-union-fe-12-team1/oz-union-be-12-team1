...
class TodosRepository:
    ...
    async def get_todo_by_id(todo_id: int) -> Optional[Todo]:
        ...
        """
        todo: Optional[Todo] = await Todo.get_or_none(id=todo_id, deleted_at=None)
        return todo
    ...
    async def get_todos_by_user(user_id: int) -> List[Todo]:
        ...
        - Soft Delete 된 데이터는 제외
        """
        todos: List[Todo] = await Todo.filter(user_id=user_id, deleted_at=None)
        return todos
    ...
    async def hard_delete_todo(todo_id: int) -> int:
        ...
        - 주로 테스트/관리자 기능용
        """
        deleted_count: int = await Todo.filter(id=todo_id).delete()
